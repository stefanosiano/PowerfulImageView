...
internal class StackBlurAlgorithm : BlurAlgorithm {
    ...
    override fun setRenderscript(renderscript: RenderScript?) {}
        ...
        if (step == 1) {
            ...
            while (y < maxY) {
                ...
                while (i <= radius) {
                    ...
                    stack_i = i + radius
                    stack[stack_i] = src[src_i]
                    sum_a += ((src[src_i].ushr(24) and 0xff) * (radius + 1 - i)).toLong()
                    ...
                    sum_in_a += (src[src_i].ushr(24) and 0xff).toLong()
                ...
                x = 0
                while (x < w) {
                    src[dst_i] = ((src[dst_i] and -0x1000000).toLong() or
                            ((sum_a * mul_sum).ushr(shr_sum.toInt()) and 0xff shl 24) or
                            ((sum_r * mul_sum).ushr(shr_sum.toInt()) and 0xff shl 16) or
                            ((sum_g * mul_sum).ushr(shr_sum.toInt()) and 0xff shl 8) or
                    ...
                    sum_out_a -= (stack[stack_i].ushr(24) and 0xff).toLong()
                    ...
                    sum_out_g += (stack[stack_i].ushr(8) and 0xff).toLong()
                    sum_out_b += (stack[stack_i] and 0xff).toLong()
                    sum_in_a -= (stack[stack_i].ushr(24) and 0xff).toLong()
    ...
    private inner class BlurTask internal constructor(private val _src: IntArray, private val _w: Int, private val _h: Int, private val _radius: Int, private val _totalCores: Int, private val _coreIndex: Int, private val _round: Int) : Callable<Void> {
        @Throws(Exception::class) override fun call(): Void? { blurIteration(_src, _w, _h, _radius, _totalCores, _coreIndex, _round); return null }
    }
}